...
public class LifeEngine implements Runnable {
		 ...
		 while (Monitor.handleRunning("get", null)) {
				...
				for (int i = 0; i<100; i++) {
					...
					for (int j = 0; j<100; j++) {
						...
						if (temporarySquares[i][j].isAlive) tryToKill(temporarySquares[i][j],i,j);
						else tryToHeal(temporarySquares[i][j],i,j);
	...
	static void tryToKill (Square square,int row,int column) {
		...
		for (int i = 0; i<neighbors.length; i++) 
			if (neighbors[i].isAlive) livingNeighbors++;
	...
	static Square[] findNeighbors(int row, int column) {
		...
		if (row == 0 && column == 0) {
			...
			for (int i = 0; i<3; i++) neighbors[i] = new Square(0,0,false);
			...
			neighbors[2].copy(temporarySquares[row+1][column]);
			 ...
			 for (int i = 0; i<5; i++) neighbors[i] = new Square(0,0,false);
			...
			for (int i = 0; i<8; i++) neighbors[i] = new Square(0,0,false);
